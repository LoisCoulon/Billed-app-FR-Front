...
describe("Given I am a user connected as en Employee", () => {
  describe("When the bill form is valid and I click on 'envoyer'", () => {
    test("Then a bill is created", async () => {
      document.body.innerHTML = NewBillUI();
      const newBill = new NewBill({
        document,
      ...
      //verify if handleSubmit has been called
      expect(handleSubmit).toHaveBeenCalled();
    });
  });
});

describe("When I navigate to the newbill page I want to post a PNG file", () => {
...
describe("When I navigate to the newbill page, I want to post a PDF file", () => {
  test("Then function handleChangeFile should be called", () => {
    const html = NewBillUI();
    document.body.innerHTML = html;
    jest.spyOn(Store.api, "post").mockImplementation(store.post);
